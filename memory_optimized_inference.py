...
print("Loading pipeline to CUDA...")
pipeline = InferencePipeline(config, device="cuda")
pipeline.to(device="cuda", dtype=dtype)

print("Loading checkpoint...")
state_dict = torch.load(os.path.join(args.checkpoint_folder, "model.pt"), map_location="cpu")['model']
...
for prompt_index in tqdm(range(len(dataset))):
    ...
    try:
        ...
        print("Min/Max:", video.min(), video.max())
        # video = ((video + 1.0) * 127.5).round().clip(0, 255).astype(np.uint8)
        video = (video + 1.0) / 2.0
        export_to_video(
            video, 
